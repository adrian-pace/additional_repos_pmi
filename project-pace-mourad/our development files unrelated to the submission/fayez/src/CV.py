...
def parameter_search(estimator, param_grid, X_train, y_train, cv=5, scoring='accuracy', verbose=2):
    """
    ...
    gsCV = GridSearchCV(estimator, param_grid, cv=cv, scoring=scoring, verbose=verbose)
    ...
    if len(param_grid)==1:
        plot_validation_curve(param_name, param_range, train_scores_mean, val_scores_mean, train_scores_std, val_scores_std,
                              scoring)
    else:
        plot_2D_validation(param_range, param_name, val_scores_mean)
