...
def parameter_search(estimator, param_grid, X_train, y_train, cv=5, scoring='accuracy', verbose=2,save_file=None,n_jobs=-1):
    """
    Search a parameter with GridSearchCV
    :param estimator: 
    ...
    #param_name = param_grid.keys()
    
    #param_range = param_grid.values()
    ...
    if len(param_grid)==1:
            plot_validation_curve(param_name[0], param_range[0], train_scores_mean, val_scores_mean, train_scores_std, val_scores_std,scoring,save_file=save_file)
    else:
...
def parameter_search_feature_selection(estimator, feature_selector, param_name, param_range, X, y, cv=5,
    ...
    plot_validation_curve(param_name, param_range, np.mean(train_scores, axis=1), val_mean,
                          np.std(train_scores, axis=1), np.std(val_scores, axis=1), scoring=scoring,save_file=save_file)
